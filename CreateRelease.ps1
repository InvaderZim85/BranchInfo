...
# Main entry point
# ===========================
$projectFile = "src\BranchInfo\BranchInfo.csproj"
...
try {
    ...
    # Create the release
    & $dotnet publish "src/BranchInfo.sln" -p:PublishProfile=src/BranchInfo/Properties/PublishProfiles/FolderProfile

    # Create the release archive
    $sourceDir = Join-Path -Path $currentLocation -ChildPath "src\BranchInfo\bin\Release\net8.0-windows\publish\win-x64\*"
    ...
    $targetPath = Join-Path -Path $releaseDir -ChildPath "BranchInfo_v$newVersion.zip"
